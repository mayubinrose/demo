...
#性别分析
select sex, count(*) from t_facefeature where face_id in (select face_id from t_entrytime where user_id="1" and  str_to_date("2018-9-12", '%Y-%m-%d')<= str_to_date(entry_time, '%Y-%m-%d') and str_to_date("2018-10-23", '%Y-%m-%d') >= str_to_date(entry_time, '%Y-%m-%d')) group by sex;
